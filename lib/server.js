...
if (cluster.isMaster) {
  ...
  d.run(function(){
    ...
    httpApi.get('/feeds/:feed', function(req, res) {

      FeedDB.load({name: req.param('feed')}, function(err, feed) {
        if(err) {
          res.status(500).send(err)
        } else {
          res.send(feed)
    ...
    httpApi.get('/feeds/:feed/:deviceId/stats', function(req, res) {
      ...
      getFeedDB(feedName, function(err, feedDB) {
        ...
        feedDB.count(filter, function(err, result) {
          if(err) {
            res.status(500).send(err)
          } else {
            res.send({
    ...
    httpApi.get('/feeds/:feed/:deviceId/:aggrFunction/:fromTimestamp/:toTimestamp', function(req, res) {
      ...
      dataFeed.getConfig(FeedDB, feed, function(err, feedConfig) {
        ...
        if(!feedConfig) return res.send(new Error('could not find feed config ' + feed))
